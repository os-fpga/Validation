...
function compile () {
    ...
    [ -z "$ip_name" ] && design_path=`find $temp -type f -iname "$design.vp"` || echo ""
    if [ -z "$design_path" ]
    then
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo "configure_ip $ip_name"_v1_0" -mod_name=$design -Pdata_width=32 -Paddr_width=16 -Pid_width=32 -Pa_pip_out=0 -Pb_pip_out=0 -Pa_interleave=0 -Pb_interleave=0 -out_file ./$design.vp">>raptor_tcl.tcl
    ...
    [ -z "$ip_name" ] && echo "" || echo "add_design_file ./rapidsilicon/ip/$ip_name/v1_0/$design/src/$design.vp">>raptor_tcl.tcl
    ...
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/$design.vp">>raptor_tcl.tcl || echo "" 
    ##vary design to design
...
#finding the co simulation testbench for the design given
    tb_path=`find $PWD -type f -iname "co_sim_$design.vp"`
    if [ -z "$tb_path" ]
    then
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        (cd ../../rtl && timeout 4m vcs -sverilog $primitive_sim $design_path $post_synth_netlist_path $tb_path -y $directory_path +define+VCS_MODE=1 -full64 -debug_all -lca -kdb && timeout 5m ./simv && mv simv *.vcd *.key *.log verdi_config_file csrc simv.daidir -t ../results_dir/$design\_$tool_name\_post_synth_files) 2>&1 | tee post_synth_sim.log
		while read line; do
    ...
    then
        ...
        #timeout 4m  vcs -sverilog $primitive_sim $latch_sim /home/users/abdulhameed.akram/Documents/Compiler_validation_team/accumulator/primitives.v $TDP18K_FIFO $ufifo_ctl $sram1024x18 $design_path $post_route_netlist_path $tb_path -y $directory_path +define+VCS_MODE=1 -full64 -debug_all 2>&1 | tee post_route_sim.log
