...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/carry_infer.v">>raptor_tcl.tcl || echo "" 
    ##vary design to design

    echo "set_top_module $design">>raptor_tcl.tcl 
    [ -z "$ip_name" ] && echo "set_top_module carry_infer">>raptor_tcl.tcl || echo "" 
    ...
    if [ "$post_synth_sim" == true ] || [ "$post_route_sim" == true ] || [ "$bitstream_sim" == true ]; then
        echo "add_simulation_file ./sim/co_sim_tb/co_sim_carry_infer.v">>raptor_tcl.tcl 
        echo "set_top_testbench co_sim_carry_infer">>raptor_tcl.tcl 
    else
        echo ""
    fi
