...
#creating a tcl file to run raptor flow 
    ...
    echo "set script_path [ file dirname [ file normalize [ info script ] ] ]">>raptor_tcl.tcl
    echo "source \$script_path/copy.tcl">>raptor_tcl.tcl
    echo "">>raptor_tcl.tcl
    echo "set projName incr_comp">>raptor_tcl.tcl
    echo "set tempFile \$projName/tmp.v">>raptor_tcl.tcl
    echo "">>raptor_tcl.tcl
    echo "create_design \$projName">>raptor_tcl.tcl
    echo "target_device GEMINI">>raptor_tcl.tcl
    echo "">>raptor_tcl.tcl
    echo "file copy -force \$script_path/and2.v \$tempFile">>raptor_tcl.tcl
    echo "">>raptor_tcl.tcl
    echo "add_design_file \$tempFile">>raptor_tcl.tcl
    echo "set_top_module and2">>raptor_tcl.tcl
    echo "# cleanup before test">>raptor_tcl.tcl
    echo "synthesize clean">>raptor_tcl.tcl
    echo "synthesize">>raptor_tcl.tcl
    echo "">>raptor_tcl.tcl
    echo "set blifPath \$projName/run_1/synth_1_1/synthesis">>raptor_tcl.tcl
    echo "file copy -force \$blifPath/incr_comp_post_synth.blif \$blifPath/incr_comp_post_synth.blif1">>raptor_tcl.tcl
    echo "# wait 1 sec since mtime will be the same for \$tempFile and *.blif">>raptor_tcl.tcl
    echo "after 1000">>raptor_tcl.tcl
    echo "copyContent \$script_path/and2_modified.v \$tempFile">>raptor_tcl.tcl
    echo "synthesize">>raptor_tcl.tcl
    echo "file copy -force \$blifPath/incr_comp_post_synth.blif \$blifPath/incr_comp_post_synth.blif2">>raptor_tcl.tcl
    echo "set diffresult [catch {exec sh -c "diff \$blifPath/incr_comp_post_synth.blif1 \$blifPath/incr_comp_post_synth.blif2"} diffresult]">>raptor_tcl.tcl
    echo "if {\$diffresult == 0} {">>raptor_tcl.tcl
    echo "    error 'Synthesis didn't resynthesized'">>raptor_tcl.tcl
    echo "}">>raptor_tcl.tcl
