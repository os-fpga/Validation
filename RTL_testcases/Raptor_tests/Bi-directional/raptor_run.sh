...
add_constraint_file="bidirectional_io.sdc" #Sets SDC + location constraints  Constraints: set_pin_loc, set_mode, all SDC Standard commands
...
pnr_netlist_lang="verilog" #blif, edif, verilog
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "read_netlist ./rtl/bidirectional_io_gate.v">>raptor_tcl.tcl || echo "" 
    ##vary design to design
