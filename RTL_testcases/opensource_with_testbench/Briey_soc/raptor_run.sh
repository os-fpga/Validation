...
#creating a tcl file to run raptor flow 
    ...
    # echo "add_simulation_file -V_2001 sim/basic_tb/bcpu_alu32_tb.v">>raptor_tcl.tcl
    # echo "set_top_testbench BitSerialCPU32_TB">>raptor_tcl.tcl
    ...
    # echo "simulation_options \"iverilog\" \"simulation\" \"--stop-time=1000ns\"">>raptor_tcl.tcl
    # echo "simulate \"rtl\" \"iverilog\" rtl_sim.vcd">>raptor_tcl.tcl
...
# sed -i -e "s|PATH|$PWD/rtl|g" rtl/*.v
# sed -i -e "s|PATH|$PWD/rtl|g" sim/basic_tb/adv_dbg_jsp_tb.v
...
#finding the co simulation testbench for the design given
    tb_path=`find $PWD -type f -iname "adv_dbg_jsp_tb.v"`
    if [ -z "$tb_path" ]
    then
