...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/InvMixColumns.v">>raptor_tcl.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/InvShiftRows.v">>raptor_tcl.tcl || echo "" 
    [ -z "$ip_name" ] && echo "add_design_file ./rtl/InverseSubBytes.v">>raptor_tcl.tcl || echo "" 
    ...
    if [ "$post_synth_sim" == true ] || [ "$post_route_sim" == true ] || [ "$bitstream_sim" == true ]; then
        echo "add_simulation_file ./sim/co_sim_tb/co_sim_$design.v rtl/$design.v rtl/AddRoundKey.v rtl/InvMixColumns.v rtl/InvShiftRows.v rtl/InverseSubBytes.v rtl/KeyExpansion.v">>raptor_tcl.tcl 
