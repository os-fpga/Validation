...
bitstream_sim=true #design_level
...
add_constraint_file="clk_constraints.sdc" #Sets SDC + location constraints  Constraints: set_pin_loc, set_mode, all SDC Standard commands
...
bitstream="write_xml pb_pin_fixup" #force
...
# ./rtl/$design.v 
    ...
    echo "add_constraint_file pin_map.pin">>raptor_tcl.tcl #design_level
    ##vary design to design
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
   ...
   then
    ...
    function simulate_bitstream() {
        bitstream_tb_path=$(find "$(realpath ../sim)" -type f -iname "${design}_include_netlists.v")
        
        [ ! -d $design\_$tool_name\_bitstream_sim_files ] && mkdir $design\_$tool_name\_bitstream_sim_files
        [ -d $design\_$tool_name\_bitstream_sim_files ] && cd $design\_$tool_name\_bitstream_sim_files
        ...
        then
            ...
            cp -R /nfs_project/castor/castor_release/v1.6.204/k6n8_TSMC16nm_7.5T/FPGA62x44_gemini_compact_latch_pnr/fabric_task/run001/k6n8_vpr/top/MIN_ROUTE_CHAN_WIDTH/SRC/ ../../../../SRC
            cp -R /nfs_project/castor/castor_release/v1.6.204/k6n8_TSMC16nm_7.5T/CommonFiles/task/CustomModules/ ../../../../SRC/CustomModules
            echo "SRC folder is copied"
        ...
        python3 ../../../../../scripts/force.py $design

        # cd $design/$design\_golden/$design\_$tool_name\_bitstream_sim_files
        start_bitstream=`date +%s`
        vcs -sverilog $bitstream_tb_path -full64 -debug_all -lca -kdb | tee bitstream_sim.log
        ./simv | tee -a bitstream_sim.log
        # iverilog -g2012 -DIVERILOG=1 -o $design $bitstream_tb_path | tee bitstream_sim.log
        # vvp ./$design | tee bitstream_sim.log
...
parse_cga
    ...
    if [[ $bitstream_sim == true ]]
    then
        echo "BITSTREAM SIMULATION"
        simulate_bitstream
    fi
