...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        (cd ../../rtl && timeout 4m vcs -sverilog -timescale=1ns/1ps $cell_path $bram_sim $sim_lib $primitive_sim $latch_sim $TDP18K_FIFO $ufifo_ctl $sram1024x18 $dsp_sim $primitive $design_path ../results_d$post_route_netlist_path $route_tb_path +incdir+$directory_path -y $directory_path +libext+.v +define+VCS_MODE=1 -full64 -debug_all -lca -kdb && timeout 5m ./simv && mv simv *.vcd *.key *.log vcs.log verdi_config_file csrc simv.daidir -t ../results_dir/$design\_$tool_name\_post_route_files) 2>&1 | tee post_route_sim.log
