...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "add_design_file -SV_2009 ./rtl/$design.v">>raptor_tcl.tcl || echo "" 
    ##vary design to design
    ...
    ##vary design to design
    echo "add_simulation_file -SV_2009 ./rtl/syn_tb.v">>raptor_tcl.tcl
...
parse_cga exit 1; }
    ...
    primitive=`find $library -wholename "*/rapidsilicon/genesis3/primitives.v"`
    compile_opts=$1    
...
#finding the co simulation route testbench for the design given
    route_tb_path=`find $PWD -type f -iname "co_sim_route_$design.v"`
    if [ -z "$route_tb_path" ]
    then
        echo "No such Test Bench for $design"
        # exit 
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        (cd ../../rtl && timeout 4m vcs -sverilog -timescale=1ns/1ps $cell_path $primitive $design_path ../results_d$post_route_netlist_path $route_tb_path -y $directory_path +define+VCS_MODE=1 -full64 -debug_all -lca -kdb && timeout 5m ./simv && mv simv *.vcd *.key *.log vcs.log verdi_config_file csrc simv.daidir -t ../results_dir/$design\_$tool_name\_post_route_files) 2>&1 | tee post_route_sim.log
