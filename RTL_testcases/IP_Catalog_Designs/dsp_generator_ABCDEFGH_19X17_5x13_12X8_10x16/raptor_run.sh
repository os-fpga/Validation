...
design="dsp_ABCDEFGH_19x17_5x13_12x8_10x16"
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pequation=AxB+CxD+ExF+GxH -Pa_width=19 -Pb_width=17 -Pc_width=5 -Pd_width=13 -Pe_width=12 -Pf_width=8 -Pg_width=10 -Ph_width=16 -Preg_in=1 -Preg_out=1 -out_file ./$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[18:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[16:0\]b/g" sim/dsp_test.v
        sed -i '26i reg [4:0]c; reg [12:0] d; reg [11:0] e; reg [7:0] f; reg [9:0] g; reg[15:0] h; reg [4:0]c1; reg [12:0] d1; reg [11:0] e1; reg [7:0] f1; reg [9:0] g1; reg[15:0] h1;' sim/dsp_test.v
        sed -i '66i c <= $random; d <= $random; e <= $random; f <= $random; g <= $random; h <= $random;' sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[39:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[39:0\]z/g" sim/dsp_test.v
        sed -i "s/dsp(a, b, z)/dsp(a, b,c, d, e, f, g, h, z)/g" sim/dsp_test.v
        sed -i '105i input [4:0]c; input [12:0] d; input [11:0]e; input [7:0] f; input [9:0]g; input [15:0] h;' sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[99:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {19{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {17{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {19{1'b0}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b0}}/b <= {17{1'b0}}/g" sim/dsp_test.v
        ...
        sed -i "59i c <= {5{1'b1}}; d <= {13{1'b1}}; e <= {12{1'b1}}; f <= {8{1'b1}}; g <= {10{1'b1}}; h <= {16{1'b1}};" sim/dsp_test.v
        sed -i "63i c <= {5{1'b0}}; d <= {13{1'b0}}; e <= {12{1'b0}}; f <= {8{1'b0}}; g <= {10{1'b0}}; h <= {16{1'b0}};" sim/dsp_test.v
