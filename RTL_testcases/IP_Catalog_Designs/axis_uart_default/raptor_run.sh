...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pdata_width=8 -out_file $IP_PATH/$design">>raptor_tcl.tcl
    ...
    echo 'exec sed -i [list -e $sed_script] test_uart_rx.py' >> raptor_tcl.tcl
    echo "exec sed -i {29i\import glob} test_uart_rx.py" >> raptor_tcl.tcl
    ...
    echo 'exec sed -i [list -e $sed_script] test_uart_rx.py' >> raptor_tcl.tcl
    echo "set sed_script {s|../src/\*\.v|$main_path/results_dir/$design/run_1/synth_1_1/synthesis/${design}_post_synth.v|g}" >> raptor_tcl.tcl
    echo 'exec sed -i [list -e $sed_script] test_uart_tx.py' >> raptor_tcl.tcl
    echo "exec sed -i {29i\import glob} test_uart_tx.py" >> raptor_tcl.tcl
    echo "set sed_script {s|srcs = \[\]|srcs = []\nsrcs += glob.glob(\"$primitive_sim_path/*.v\")\nsrcs.remove(\"$primitive_sim_path/SOC_FPGA_TEMPERATURE.v\")|}" >> raptor_tcl.tcl
    echo 'exec sed -i [list -e $sed_script] test_uart_tx.py' >> raptor_tcl.tcl
    echo "set sed_script {s|iverilog|iverilog -g2012|}" >> raptor_tcl.tcl
    echo 'exec sed -i [list -e $sed_script] test_uart_tx.py' >> raptor_tcl.tcl
    echo "exec make clean" >> raptor_tcl.tcl
    echo "exec make MODULE_NAME=$design > post_synth_sim.log" >> raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        cd ../results_dir/rapidsilicon/ip/$ip_name/v1_0/$design/sim && sed -i '/srcs.append/d' test_uart*.py
        sed -i "38i srcs.append(\"../src/*.v\")" test_uart*.py
        sed -i "38i srcs.append(\"%s.v\" % testbench)" test_uart*.py
        make clean && (echo -e "\n" && timeout 4m ./sim.sh) 2>&1 | tee pre_synth_sim.log
    ...
    then
        cd ../results_dir/rapidsilicon/ip/$ip_name/v1_0/$design/sim && sed -i '/srcs.append/d' test_uart*.py
        sed -i "38i srcs.append(\"../../../../../../$post_synth_netlist_path\")" test_uart*.py
        sed -i "38i srcs.append(\"$bram_sim\")" test_uart*.py
        sed -i "38i srcs.append(\"$cell_path\")" test_uart*.py
        sed -i "38i srcs.append(\"$dsp_sim\")" test_uart*.py
        sed -i "38i srcs.append(\"$dsp_map\")" test_uart*.py
        # sed -i "38i srcs.append(\"$dsp_final_map\")" test_uart*.py
        sed -i "38i srcs.append(\"$primitive_sim\")" test_uart*.py
        sed -i "38i srcs.append(\"$TDP18K_FIFO\")" test_uart*.py
        sed -i "38i srcs.append(\"$ufifo_ctl\")" test_uart*.py
        sed -i "38i srcs.append(\"$sram1024x18\")" test_uart*.py
        sed -i "38i srcs.append(\"%s.v\" % testbench)" test_uart*.py
        sed -i "s/\baxis_crosspoint\b/axis_interconnect_default_post_synth/g" test_axis*.v
        # timeout 4m make
        # cd ..
    fi
