...
design="vexriscv_cpu_mmu"
    ip_name="vexriscv_cpu"
tool_name="iverilog" 
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -version=v1_0 -Pvariant=Cache_MMU -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    if [[ $tool_name == "verilator" ]] && [[ $ip_level == true ]]
    then
        mkdir ../results_dir/$design\_$tool_name\_pre_synth_files
        cd ../results_dir/rapidsilicon/ip/$ip_name/v1_0/$design/sim && make clean && (echo -e "\n" && timeout 4m ./sim.sh) 2>&1 | tee pre_synth_sim.log
