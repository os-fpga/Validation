...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pdata_width=32 -Paddr_width=16 -Pfilter_len=4 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        cd ../results_dir/rapidsilicon/ip/$ip_name/v1_0/$design/sim && sed -i '/srcs.append/d' test_$ip_name*.py
        sed -i "38i srcs.append(\"../src/*.v\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"%s.v\" % testbench)" test_$ip_name*.py
    ...
    then
        cd ../results_dir/rapidsilicon/ip/$ip_name/v1_0/$design/sim && sed -i '/srcs.append/d' test_$ip_name*.py
        sed -i "38i srcs.append(\"../../../../../../$post_synth_netlist_path\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$bram_sim\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$cell_path\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$dsp_sim\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$dsp_map\")" test_$ip_name*.py
        # sed -i "38i srcs.append(\"$dsp_final_map\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$primitive_sim\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$TDP18K_FIFO\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$ufifo_ctl\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"$sram1024x18\")" test_$ip_name*.py
        sed -i "38i srcs.append(\"%s.v\" % testbench)" test_$ip_name*.py
