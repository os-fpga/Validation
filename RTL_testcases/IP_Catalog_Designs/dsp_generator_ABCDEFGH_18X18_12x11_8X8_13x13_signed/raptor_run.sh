...
design="dsp_ABCDEFGH_18X18_12x11_8X8_13x13_signed"
    ip_name="dsp_generator"
tool_name="iverilog" 
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pequation=AxB+CxD+ExF+GxH -Pa_width=18 -Pb_width=18 -Pc_width=12 -Pd_width=11 -Pe_width=8 -Pf_width=8 -Pg_width=13 -Ph_width=13 -Preg_in=0 -Preg_out=1 -Punsigned=0 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[17:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[17:0\]b/g" sim/dsp_test.v
        sed -i '26i reg [11:0]c; reg [10:0] d; reg [7:0] e, f; reg [12:0] g, h;reg [11:0]c1; reg [10:0] d1; reg [7:0] e1, f1; reg [12:0] g1, h1;' sim/dsp_test.v
        sed -i '66i c <= $random; d <= $random; e <= $random; f <= $random; g <= $random; h <= $random;' sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[37:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[37:0\]z/g" sim/dsp_test.v
        sed -i "s/dsp(a, b, z)/dsp(a, b,c, d, e, f, g, h, z)/g" sim/dsp_test.v
        sed -i '105i input signed [11:0]c; input signed [10:0] d; input signed [7:0]e; input signed [7:0] f; input signed [12:0]g; input signed [12:0] h;' sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[99:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {18{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {18{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {18{1'b0}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b0}}/b <= {18{1'b0}}/g" sim/dsp_test.v
        sed -i "s/a\*b/a\*b+c\*d+e\*f+g\*h/g" sim/dsp_test.v
        sed -i "s/dsp_wrapper dut1(.a(a),.b(b),.z(z1))/$design dut1(.a(a),.b(b),.z(z1), .c(c),.d(d), .e(e), .f(f), .g(g), .h(h), .clk(clk1), .reset(reset))/g" sim/dsp_test.v
        sed -i "s/dsp dut2(.a(a),.b(b),.z(z2))/dsp dut2(.a(a1),.b(b1),.z(z2), .c(c1),.d(d1), .e(e1), .f(f1), .g(g1), .h(h1))/g" sim/dsp_test.v
        sed -i "59i c <= {12{1'b1}}; d <= {11{1'b1}}; e <= {8{1'b1}}; f <= {8{1'b1}}; g <= {13{1'b1}}; h <= {13{1'b1}};" sim/dsp_test.v
        sed -i "63i c <= {12{1'b0}}; d <= {11{1'b0}}; e <= {8{1'b0}}; f <= {8{1'b0}}; g <= {13{1'b0}}; h <= {13{1'b0}};" sim/dsp_test.v
        sed -i "s/input   \[17:0\]a/input signed  \[17:0\]a/g" sim/dsp_test.v
        sed -i "s/input  \[17:0\]b/input signed \[17:0\]b/g" sim/dsp_test.v
        sed -i "s/b1 <= b;/b1 <= b; c1 <= c; d1 <= d; e1 <= e; f1 <= f; g1 <= g; h1 <= h;/g" sim/dsp_test.v
