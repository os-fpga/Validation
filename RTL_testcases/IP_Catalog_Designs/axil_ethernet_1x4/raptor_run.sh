...
design="axil_ethernet_1x4"
    ip_name="axil_ethernet"
tool_name="iverilog" 
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -version=v1_0 -Pcore_ntxslots=1 -Pcore_nrxslots=4 -Pcore_phy=model -Pcore_bus_endianness=little -out_file ./$design">>raptor_tcl.tcl
