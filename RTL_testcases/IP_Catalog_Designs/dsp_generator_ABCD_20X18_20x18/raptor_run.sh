...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pequation=AxB+CxD -Pa_width=20 -Pb_width=18 -Pc_width=20 -Pd_width=18 -Preg_in=0 -Preg_out=1 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i '26i reg [19:0]c; reg [17:0] d; reg [19:0]c1; reg [17:0] d1;' sim/dsp_test.v
        sed -i '66i c <= $random; d <= $random;' sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[38:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[38:0\]z/g" sim/dsp_test.v
        sed -i "s/dsp(a, b, z)/dsp(a, b,c, d, z)/g" sim/dsp_test.v
        sed -i '105i input [19:0]c; input [17:0] d;' sim/dsp_test.v
        ...
        sed -i "59i c <= {20{1'b1}}; d <= {18{1'b1}};" sim/dsp_test.v
        sed -i "63i c <= {20{1'b0}}; d <= {18{1'b0}};" sim/dsp_test.v
