...
design="ocm_sp_32x1024"
    ip_name="on_chip_memory"
tool_name="iverilog" 
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pmemory_type=Single_Port -Pdata_width=32 -Pwrite_depth=1024 -Pcommon_clk=0 -Pbram=0 -out_file ./$design">>raptor_tcl.tcl
    ...
    if [ "$post_synth_sim" == true ] || [ "$post_route_sim" == true ] || [ "$bitstream_sim" == true ]; then
        echo "add_simulation_file add_simulation_file $main_path/results_dir/$design/run_1/IPs/rapidsilicon/ip/$ip_name/v1_0/$design/sim/testbench.v add_simulation_file $main_path/results_dir/$design/run_1/IPs/rapidsilicon/ip/$ip_name/v1_0/$design/sim/on_chip_memory_v1_0.v">>raptor_tcl.tcl 
