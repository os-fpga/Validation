...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pmemory_type=True_Dual_Port -Pdata_width=20 -Pwrite_depth=32768 -Pcommon_clk=1 -Pbram=1 -out_file $IP_PATH/$design">>raptor_tcl.tcl
