...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Poptimization=Performance -Pcoefficients=45,67,101,45.54,-90.23,56 -Pcoefficient_fractional_bits=8 -Pcoefficient_width=20 -out_file $IP_PATH/$design">>raptor_tcl.tcl
    ...
    if [ "$post_synth_sim_hard_code" == true ] || [ "$post_route_sim" == true ] || [ "$bitstream_sim" == true ]; then
        echo "add_simulation_file $main_path/results_dir/$design/run_1/IPs/rapidsilicon/ip/$ip_name/v1_0/$design/sim/testbench.v $main_path/results_dir/$design/run_1/IPs/rapidsilicon/ip/$ip_name/v1_0/$design/sim/fir_golden.v">>raptor_tcl.tcl 
    else
        echo ""
    fi
