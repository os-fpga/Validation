...
design="fir_generator_performance_six_coefficients_signed"
    ip_name="fir_generator"
tool_name="iverilog" 
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Poptimization=Performance -Pcoefficients=45,67,101,45.54,-90.23,56 -Pcoefficient_fractional_bits=8 -Pcoefficient_width=20 -out_file ./$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/$design/$design\_post_synth/g" sim/testbench.v
        sed -i '/timescale/d' sim/testbench.v
        (timeout 4m vcs ../../../../../$design/$design\_post_synth.v $primitive_sim $bram_sim $cell_path $dsp_map $TDP18K_FIFO $ufifo_ctl $sram1024x18 $dsp_sim sim/testbench.v -sverilog -debug_access -full64 && timeout 5m ./simv) 2>&1 | tee post_synth_sim.log
