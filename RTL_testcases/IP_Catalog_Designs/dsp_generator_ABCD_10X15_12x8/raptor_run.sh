...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pequation=AxB+CxD -Pa_width=10 -Pb_width=15 -Pc_width=12 -Pd_width=8 -Preg_in=1 -Preg_out=0 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[9:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[14:0\]b/g" sim/dsp_test.v
        sed -i '26i reg [11:0]c; reg [7:0] d; reg [11:0]c1; reg [7:0] d1;' sim/dsp_test.v
        ...
        sed -i '105i input [11:0]c; input [7:0] d;' sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[99:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {10{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {15{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {10{1'b0}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b0}}/b <= {15{1'b0}}/g" sim/dsp_test.v
        ...
        sed -i "59i c <= {12{1'b1}}; d <= {8{1'b1}};" sim/dsp_test.v
        sed -i "63i c <= {12{1'b0}}; d <= {8{1'b0}};" sim/dsp_test.v
