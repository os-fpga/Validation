...
design="dsp_ABCD_15x10_7x2_signed"
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pequation=AxB+CxD -Pa_width=15 -Pb_width=10 -Pc_width=7 -Pd_width=2 -Preg_in=0 -Preg_out=1 -Punsigned=0 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[14:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[9:0\]b/g" sim/dsp_test.v
        sed -i '26i reg [6:0]c; reg [1:0] d; reg [6:0]c1; reg [1:0] d1;' sim/dsp_test.v
        sed -i '66i c <= $random; d <= $random;' sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[24:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[24:0\]z/g" sim/dsp_test.v
        sed -i "s/dsp(a, b, z)/dsp(a, b,c, d, z)/g" sim/dsp_test.v
        sed -i '105i input signed [6:0]c; input signed [1:0] d;' sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[99:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {15{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {10{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {15{1'b0}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b0}}/b <= {10{1'b0}}/g" sim/dsp_test.v
        sed -i "s/a\*b/a\*b+c\*d/g" sim/dsp_test.v
        sed -i "s/dsp_wrapper dut1(.a(a),.b(b),.z(z1))/$design dut1(.a(a),.b(b),.z(z1), .c(c),.d(d), .clk(clk1), .reset(reset))/g" sim/dsp_test.v
        sed -i "s/dsp dut2(.a(a),.b(b),.z(z2))/dsp dut2(.a(a1),.b(b1),.z(z2), .c(c1),.d(d1))/g" sim/dsp_test.v
        sed -i "59i c <= {7{1'b1}}; d <= {2{1'b1}};" sim/dsp_test.v
        sed -i "63i c <= {7{1'b0}}; d <= {2{1'b0}};" sim/dsp_test.v
        sed -i "s/input   \[14:0\]a/input signed  \[14:0\]a/g" sim/dsp_test.v
        sed -i "s/input  \[9:0\]b/input signed \[9:0\]b/g" sim/dsp_test.v
        sed -i "s/b1 <= b;/b1 <= b; c1 <= c; d1 <= d;/g" sim/dsp_test.v
