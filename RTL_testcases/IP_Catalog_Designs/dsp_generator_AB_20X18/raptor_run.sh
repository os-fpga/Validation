...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Preg_in=0 -Preg_out=1 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[78:0\]z1/\[27:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[27:0\]z/g" sim/dsp_test.v
