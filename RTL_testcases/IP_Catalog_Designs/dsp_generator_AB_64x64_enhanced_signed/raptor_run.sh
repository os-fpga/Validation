...
design="dsp_AB_64x64_Enhanced_signed"
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pa_width=64 -Pb_width=64 -Pfeature=Enhanced -Punsigned=0 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[63:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[63:0\]b/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[127:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[127:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {64{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {64{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {64{1'b0}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b0}}/b <= {64{1'b0}}/g" sim/dsp_test.v
        sed -i "s/dsp_wrapper/$design/g" sim/dsp_test.v
        sed -i "s/input   \[63:0\]a/input signed  \[63:0\]a/g" sim/dsp_test.v
        sed -i "s/input  \[63:0\]b/input signed \[63:0\]b/g" sim/dsp_test.v
