...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pa_width=72 -Pb_width=72 -Pfeature=Pipeline -Punsigned=1 -out_file $IP_PATH/$design">>raptor_tcl.tcl
    [ -z "$ip_name" ] && echo "" || echo "ipgenerate">>raptor_tcl.tcl

    # [ -z "$ip_name" ] && echo "" || echo "add_include_path $IP_PATH/$IP_PATH/rapidsilicon/ip/$ip_name/v1_0/$design/src/">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[71:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[71:0\]b/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[143:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[143:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {72{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {72{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {72{1'b0}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b0}}/b <= {72{1'b0}}/g" sim/dsp_test.v
