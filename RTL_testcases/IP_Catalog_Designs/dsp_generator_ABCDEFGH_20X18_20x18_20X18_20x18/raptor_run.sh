...
design="dsp_ABCDEFGH_20X18_20x18_20X18_20x18"
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pequation=AxB+CxD+ExF+GxH -Pa_width=20 -Pb_width=18 -Pc_width=20 -Pd_width=18 -Pe_width=20 -Pf_width=18 -Pg_width=20 -Ph_width=18 -Preg_in=0 -Preg_out=1 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[19:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[17:0\]b/g" sim/dsp_test.v
        sed -i '26i reg [19:0]c, c1; reg [17:0] d, d1; reg [19:0] e, e1; reg [17:0] f, f1; reg [19:0] g, g1; reg[17:0] h, h1;' sim/dsp_test.v
        sed -i '66i c <= $random; d <= $random; e <= $random; f <= $random; g <= $random; h <= $random;' sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[40:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[40:0\]z/g" sim/dsp_test.v
        sed -i "s/dsp(a, b, z)/dsp(a, b,c, d, e, f, g, h, z)/g" sim/dsp_test.v
        sed -i '105i input [19:0]c; input [17:0] d; input [19:0]e; input [17:0] f; input [19:0]g; input [17:0] h;' sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[99:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {20{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {18{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {20{1'b0}}/g" sim/dsp_test.v
        ...
        sed -i "59i c <= {20{1'b1}}; d <= {18{1'b1}}; e <= {20{1'b1}}; f <= {18{1'b1}}; g <= {20{1'b1}}; h <= {18{1'b1}};" sim/dsp_test.v
        sed -i "63i c <= {20{1'b0}}; d <= {18{1'b0}}; e <= {20{1'b0}}; f <= {18{1'b0}}; g <= {20{1'b0}}; h <= {18{1'b0}};" sim/dsp_test.v
