...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pmemory_type=Single_Port -Pdata_width=32 -Pwrite_depth=2000 -Pcommon_clk=0 -Pbram=1 -Pfile_path=$find_mem_file -out_file $IP_PATH/$design">>raptor_tcl.tcl
