...
design="axis_interconnect_wrapper"
    ip_name="axis_interconnect"
tool_name="iverilog" 
...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pm_count=2 -Ps_count=2 -Pdata_width=1024 -Plast_en=1 -Pid_en=1 -Pid_width=8 -Pdest_en=1 -Pdest_width=8 -Puser_en=1 -Puser_width=1 -out_file $IP_PATH/$design">>raptor_tcl.tcl
    ...
    [ -z "$ip_name" ] && echo "" || echo "add_design_file $IP_PATH/rapidsilicon/ip/$ip_name/v1_0/$design/src/$design.v">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        cd ../results_dir/rapidsilicon/ip/$ip_name/v1_0/$design/sim && sed -i '/srcs.append/d' test_*.py
        sed -i "38i srcs.append(\"../src/*.v\")" test_*.py
        sed -i "38i srcs.append(\"%s.v\" % testbench)" test_*.py
    ...
    then
        cd ../results_dir/rapidsilicon/ip/$ip_name/v1_0/$design/sim && sed -i '/srcs.append/d' test_*.py
        sed -i "38i srcs.append(\"../../../../../../$post_synth_netlist_path\")" test_*.py
        sed -i "38i srcs.append(\"$bram_sim\")" test_*.py
        sed -i "38i srcs.append(\"$cell_path\")" test_*.py
        sed -i "38i srcs.append(\"$dsp_sim\")" test_*.py
        sed -i "38i srcs.append(\"$dsp_map\")" test_*.py
        # sed -i "38i srcs.append(\"$dsp_final_map\")" test_*.py
        sed -i "38i srcs.append(\"$primitive_sim\")" test_*.py
        sed -i "38i srcs.append(\"$TDP18K_FIFO\")" test_*.py
        sed -i "38i srcs.append(\"$ufifo_ctl\")" test_*.py
        sed -i "38i srcs.append(\"$sram1024x18\")" test_*.py
        sed -i "38i srcs.append(\"%s.v\" % testbench)" test_*.py
