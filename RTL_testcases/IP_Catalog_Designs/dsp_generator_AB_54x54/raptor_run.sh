...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pa_width=54 -Pb_width=54 -out_file $IP_PATH/$design">>raptor_tcl.tcl
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        sed -i "s/\[71:0\]a/\[53:0\]a/g" sim/dsp_test.v
        sed -i "s/\[6:0\]b/\[53:0\]b/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z1/\[107:0\]z1/g" sim/dsp_test.v
        sed -i "s/\[78:0\]z/\[107:0\]z/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b1}}/a <= {54{1'b1}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b1}}/b <= {54{1'b1}}/g" sim/dsp_test.v
        sed -i "s/a <= {20{1'b0}}/a <= {54{1'b0}}/g" sim/dsp_test.v
        sed -i "s/b <= {20{1'b0}}/b <= {54{1'b0}}/g" sim/dsp_test.v
