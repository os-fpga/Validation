...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pmemory_type=Simple_Dual_Port -Pdata_width=79 -Pwrite_depth=4096 -Pcommon_clk=1 -Pmemory_mapping=Block_RAM -out_file $IP_PATH/$design">>raptor_tcl.tcl
