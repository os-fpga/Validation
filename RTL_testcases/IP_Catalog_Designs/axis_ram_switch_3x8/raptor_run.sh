...
#creating a tcl file to run raptor flow 
    ...
    [ -z "$ip_name" ] && echo "" || echo  "configure_ip $ip_name"_v1_0" -mod_name $design -Pm_count=3 -Ps_count=8 -Pid_en=true -Ptid=true -Pram_pipeline=6 -Ptype_round_robin=false -Pcmd_fifo_depth=16 -P -version=v1_0 -out_file $IP_PATH/$design">>raptor_tcl.tcl
