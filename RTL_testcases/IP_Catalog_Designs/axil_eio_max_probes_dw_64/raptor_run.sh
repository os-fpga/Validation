...
design="axil_eio_max_probes_dw_64"
    ip_name="axil_eio"
tool_name="iverilog" 
...
if cmp --silent -- "../cksums.md5" "../newsum.md5" && [ -d $design ]; then
   ...
   timeout $timeout raptor --batch $mute_flag --script ../raptor_tcl.tcl 2>&1 | tee results.log 
    if [ ${PIPESTATUS[0]} -eq 124 ]; then
        echo -e "\nERROR: Design Compilation took $timeout. Exiting due to timeout">>raptor.log
		end_time
        parse_cga
        exit
    fi
