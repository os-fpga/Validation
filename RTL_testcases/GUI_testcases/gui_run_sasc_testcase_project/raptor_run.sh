...
cp -R $examples_path/sasc_testcase .

echo "run_project ../sasc_testcase/sasc_testcase.ospr">raptor_tcl.tcl
...
function compile () {
    ...
    timeout 10m xvfb-run --auto-servernum --server-args="-screen 0, 1280x1024x24" raptor --script ../raptor_tcl.tcl 2>&1 | tee -a results.log
