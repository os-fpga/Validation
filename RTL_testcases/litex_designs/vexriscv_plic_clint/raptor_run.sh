...
tb_path=`find $main_path -wholename "testbench.v"`
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    if [[ $tool_name == "vcs" ]] && [[ $ip_level == true ]]
    then
        (timeout 4m vcs -sverilog -timescale=1ns/1ps ../rtl/*.v ../tb/testbench.v +incdir+../rtl/ -full64 -debug_access -lca -kdb && timeout 5m ./simv && mkdir ../results_dir/$design\_$tool_name\_pre_synth_files && mv simv *.vcd *.key vcs.log pre*.log verdi_config_file -t ../results_dir/$design\_$tool_name\_pre_synth_files ) 2>&1 | tee pre_synth_sim.log
        ip_level=false
        return
    fi
    ...
    if [[ $tool_name == "iverilog" ]] && [[ $compile_opts == "ip_level" ]]
    then
        [ ! -d $design\_$tool_name\_post_synth_files ] && mkdir $design\_$tool_name\_post_synth_files
        [ -d $design\_$tool_name\_post_synth_files ] && cd $design\_$tool_name\_post_synth_files
        (cd ../../rtl && timeout 10m iverilog -g2012 -o $design $primitive_sim $sim_lib  $post_synth_netlist_path $tb_path -y $directory_path && timeout 5m vvp ./$design && mv $design tb.vcd -t ../results_dir/$design\_$tool_name\_post_synth_files) 2>&1 | tee post_synth_sim.log
