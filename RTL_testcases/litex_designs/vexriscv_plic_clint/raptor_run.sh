...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
        ...
        (cd ../../rtl && rm -rf simv.daidir csrc && timeout 4m vcs -sverilog -timescale=1ns/1ps ../tb/testbench.v $cell_path $bram_sim $sim_lib $primitive_sim $latch_sim $TDP18K_FIFO $ufifo_ctl $sram1024x18 $dsp_sim $post_synth_netlist_path -full64 -debug_access -lca -kdb && timeout 5m ./simv && mv simv *.vcd *.key *.log verdi_config_file csrc simv.daidir -t ../results_dir/$design\_$tool_name\_post_synth_files ) 2>&1 | tee post_synth_sim.log
    ...
    then
        [ ! -d $design\_$tool_name\_post_synth_files ] && mkdir $design\_$tool_name\_post_synth_files
        [ -d $design\_$tool_name\_post_synth_files ] && cd $design\_$tool_name\_post_synth_files
        (cd ../../rtl && timeout 10m iverilog -g2012 -o $design $cell_path $bram_sim $sim_lib $primitive_sim $latch_sim $TDP18K_FIFO $ufifo_ctl $sram1024x18 $dsp_sim $post_synth_netlist_path $tb_path +incdir+$directory_path -y $directory_path +libext+.v && timeout 5m vvp ./$design && mv $design tb.vcd -t ../results_dir/$design\_$tool_name\_post_synth_files) 2>&1 | tee post_synth_sim.log
