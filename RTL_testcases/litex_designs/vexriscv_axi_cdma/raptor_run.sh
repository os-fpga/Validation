...
sed -i -e "s|MEM_FILE_PATH|$PWD/rtl|g" rtl/*.v

[ -f rtl/uart_receiver.v ] && sed -i -e "s|MEM_FILE_PATH|$PWD/rtl|g" rtl/uart_receiver.v
[ -f rtl/uart_regs.v ] && sed -i -e "s|MEM_FILE_PATH|$PWD/rtl|g" rtl/uart_regs.v
[ -f rtl/uart_transmitter.v ] && sed -i -e "s|MEM_FILE_PATH|$PWD/rtl|g" rtl/uart_transmitter.v
...
#reading log file of raptor to see is synthesis failed, if not failed staring the simulation
    ...
    then
    echo $PWD
        (timeout 4m `dirname \`which raptor\``/HDL_simulator/iverilog/bin/iverilog -g2012 -o vex ../rtl/*.v ../tb/testbench.v -I../rtl/ && `dirname \`which raptor\``/HDL_simulator/iverilog/bin/timeout 5m vvp vex && mkdir ../results_dir/$design\_$tool_name\_pre_synth_files && mv *.vcd pre*.log -t ../results_dir/$design\_$tool_name\_pre_synth_files ) 2>&1 | tee pre_synth_sim.log
        ip_level=false
        return
    fi
    ...
    then
        ...
        (cd ../../rtl && rm -rf simv.daidir csrc && timeout 4m vcs -sverilog -timescale=1ns/1ps ../tb/testbench.v $primitive_sim $post_synth_netlist_path -full64 -debug_access -lca -kdb && timeout 5m ./simv && mv simv *.vcd *.key *.log verdi_config_file csrc simv.daidir -t ../results_dir/$design\_$tool_name\_post_synth_files ) 2>&1 | tee post_synth_sim.log
        ls && vcd2fst tb.vcd tb.fst --compress && rm -fr tb.vcd
        cd ..
        return
    fi
...
parse_cga
    ...
    if [[ $ip_level == true ]]
    then
        echo "only IP_LEVEL SIM"
        simulate "ip_level"
        [ -f $PWD/$design\_$tool_name\_pre_synth_files/pre_synth_sim.log ] && cat $PWD/$design\_$tool_name\_pre_synth_files/pre_synth_sim.log >> results.log
    fi
    if [[ $post_synth_sim == true ]] 
    then
        echo "post_synth $PWD"
