...
design="udp_ipv4_for_10g_ethernet"
...
#creating a tcl file to run raptor flow 
    ...
    if [ -z "$ip_name" ] 
    then
        array1=($(find $directory_path -type f -iname "*.vhd" -print0 | xargs -0 grep -l "package")) 
        array2=($(find $directory_path -type f -iname "*.vhd" -printf "%p "))
        array=("${array1[@]}" "${array2[@]}")
        for i in ${array[@]}; do
            abc=$(echo $i | sed 's/\// /g')
    ...
    echo "set_top_module udp_ip_10g">>raptor_tcl.tcl 
