...
#creating a tcl file to run raptor flow 
    ...
    if [ "$post_synth_sim" == true ]; then 
        [ "$tool_name" = "iverilog" ] && echo "simulation_options compilation icarus gate" >> raptor_tcl.tcl || echo "simulation_options compilation verilator -Wno-BLKLOOPINIT gate" >> raptor_tcl.tcl
    ...
    echo "route">>raptor_tcl.tcl  
        if [ "$post_route_sim" == true ]; then 
            [ "$tool_name" = "iverilog" ] && echo "simulation_options compilation icarus -DPNR=1 pnr" >> raptor_tcl.tcl || echo "simulation_options compilation verilator -Wno-BLKLOOPINIT -DPNR=1 pnr" >> raptor_tcl.tcl
